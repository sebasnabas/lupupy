...
def call():
    ...
    if not args.username or not args.password or not args.ip_address:
        raise Exception("Please supply username, password and ip address.")
    ...
    try:
        ...
        if args.history:
            _LOGGER.info(
                json.dumps(lupusec.get_history()["hisrows"], indent=4, sort_keys=True)
            )
